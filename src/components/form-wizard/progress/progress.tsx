...
export const Proggres: FC<ProggresProps> = ({
  clickedProggresItem: clickedPI
}) => {
  const {
    wizards,
    visitedLevel: vlvl,
    level
  } = useSelector<RootState>((state) => state.wizard) as WizardState
  ...
  const width =
    levelSize === 0
      ...
      : levelSize === 1
      ? vlvl * 100
      : (vlvl * 100) / (levelSize - 1)

  const progItms: ProggresItemProps[] = [...Array(levelSize)].map(
    (_, index) => ({
  ...
  return (
    <div className={classes.Progress}>
      <div
        className={`${classes.ProgressLinear} ${classes.ProgressLinearBack}`}
      ></div>
      <div
        style={{ width: width + '%' }}
